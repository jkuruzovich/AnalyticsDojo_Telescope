 ...
 * @namespace Calendars.parameters
 */
Calendars.parameters = {};

/**
 * Takes a set of terms, and translates them into a `parameter` object containing the appropriate find
 * and options arguments for the subscriptions's Calendars.find()
 * @param {Object} terms
 */
Calendars.parameters.get = function (terms) {

  // add this to ensure all calendar publications pass audit-arguments-check
  ...
  var parameters = Telescope.utils.deepExtend(true, {}, Calendars.views.baseParameters);

  // iterate over calendarsParameters callbacks
  parameters = Telescope.callbacks.run("calendarsParameters", parameters, terms);
  ...
  // extend sort to sort calendars by _id to break ties
...
// View Parameter
// Add a "view" property to terms which can be used to filter calendars. 
function addViewParameter (parameters, terms) {
  ...
  if (typeof Calendars.views[view] !== 'undefined')
    parameters = Telescope.utils.deepExtend(true, parameters, Calendars.views[view](terms));

  return parameters;
}
Telescope.callbacks.add("calendarsParameters", addViewParameter);

// View Parameter
// Add "after" and "before" properties to terms which can be used to limit calendars in time. 
...
Telescope.callbacks.add("calendarsParameters", addTimeParameter);

// limit the number of items that can be requested at once
function limitCalendars (parameters, terms) {
  var maxLimit = 200;
...
Telescope.callbacks.add("calendarsParameters", limitCalendars);

// hide future scheduled calendars unless "showFuture" is set to true or postedAt is already defined
function hideFutureCalendars (parameters, terms) {
...
Telescope.callbacks.add("calendarsParameters", hideFutureCalendars);